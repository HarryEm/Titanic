...
We need to add a field classifying which data are missing

```{r}
a <- colSums(is.na(testdata))+colSums(testdata=="")
a <- names(a[is.na(a)|a!=0])

missing <- paste(as.integer(!is.na(merged$Age)^!merged$Age==""),
as.integer(!is.na(merged$Fare)^!merged$Fare==""),
as.integer(!is.na(merged$Fare)^!merged$Cabin==""),sep="")
...
0 means missing and 1 means observed, so for example "010" means Age was observed, Fare was not, and Cabin was.
