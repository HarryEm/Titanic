...
# data is small so performance shouldn't be an issue
...
There are five different combinations of missing variables in the data, so the approach will be to loop over these and train five models.
...
gn <- ggplot(dftemp, aes(x = Pclass,
    ...
    labs(y = "Proportion Survived",title="Survivor split by ticket class - Normalised") +
...
g <- ggplot(merged[1:891,], aes(x=Age_range,fill=factor(Survived))) + geom_bar(pos="dodge") + labs(fill = "Survived") + labs(title="Survivor split by age group")
...
gn <- ggplot(dftemp, aes(x = Age_range,
    ...
    geom_bar(position = "dodge",stat = "identity") + 
    scale_y_continuous(labels = percent_format()) +
    labs(y = "Proportion Survived",title="Survivor split by age group - Normalised") +
    theme(legend.title=element_blank())
...
gn <- gn +facet_wrap(~Pclass) +labs(y = "Proportion Survived",fill = "Survived",title="Survivor split by class and Cabin - Normalized")
...
```{r}
qplot(merged$Fare,bins=150,fill=I("red"))
```
...
g <- g +facet_wrap(~Pclass) + labs(fill="Survived",title="Survival scatterplot of Fare and Age, Split by Class")
g
```
...
g <- g +facet_wrap(~Pclass) + labs(y = "Proportion Survived",fill = "Survived",title="Survivor split by class and Title - Normalized")
g
```
...
It appears that grouping ages has a negative effect on accuracy, but only slightly.
...
rf110 <- randomForest(factor(Survived) ~ .,
                        data = subset(dftemp[,!names(dftemp) %in% c("HasCabin")],dftemp$Missing=="110") , na.action = na.pass,nodesize=20)

rf111 <- randomForest(factor(Survived) ~ .,
                        data = subset(dftemp[,!names(dftemp) %in% c()],dftemp$Missing=="111") , na.action = na.pass,nodesize=20)

rf010 <- randomForest(factor(Survived) ~ .,
                        data = subset(dftemp[,!names(dftemp) %in% c("Age","HasCabin")],dftemp$Missing=="010") , na.action = na.pass,nodesize=20)

rf011 <- randomForest(factor(Survived) ~ .,
                        data = subset(dftemp[,!names(dftemp) %in% c("Age")],dftemp$Missing=="011") , na.action = na.pass,nodesize=20)

rf100 <- randomForest(factor(Survived) ~ .,
                        data = subset(dftemp[,!names(dftemp) %in% c("Farebracket","HasCabin")],dftemp$Missing=="100") , na.action = na.pass,nodesize=20)
...
for (i in unique(merged$Missing)){
  
 # First define which variables need to be dropped if the data was missing originally
  print(i)
  
  a <- c()
  if (strsplit(as.character(i), "")[[1]][1] == 0) {
    a <- append(a,"Age")
  } 
  if(strsplit(as.character(i), "")[[1]][2] == 0) {
    a <- append(a,"Farebracket")
  } 
  if (strsplit(as.character(i), "")[[1]][3] == 0){
    a <- append(a,"HasCabin")
  }
  ...
  dftemp <- mergedtrain[,c("Survived","Pclass","Sex","Farebracket","Age","HasCabin","Ticketsize","Embarked","Title","Missing")]
  dftemp <- dftemp[, !names(dftemp) %in% a]
  
  # Finally train all the models, calling them, for example "rf100" where Missing == 100.
  
   rf <- randomForest(factor(Survived) ~ .,
                        data = dftemp[dftemp$Missing==i,] ,na.action = na.pass,nodesize=20)
